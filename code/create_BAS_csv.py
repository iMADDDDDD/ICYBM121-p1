...
def create_csv(e13_dataset, tfp_dataset, fsf_dataset, int_dataset, twt_dataset, fak_users_file, bas_users_dataset, user_id_index, header,kind):
    ...
    with open(fak_users_file, 'r', encoding = 'utf-8') as users_file:
        ...
        for row in users_reader:
           ...
           dataset = row[1]
           if dataset == 'fsf':
              copy_part_of_csv(fsf_dataset, bas_users_dataset, user_id, user_id_index, kind)
           elif dataset == 'int':
              copy_part_of_csv(int_dataset, bas_users_dataset, user_id, user_id_index, kind)
           elif dataset == 'twt': 
              copy_part_of_csv(twt_dataset, bas_users_dataset, user_id, user_id_index, kind)
...
def copy_part_of_csv(dataset1, dataset2, user_id, user_id_index, kind):
    if kind == 'users':
        df_csv = pandas.read_csv(dataset1, index_col=int(user_id_index))
        rows_user = df_csv.loc[int(user_id)]
        #print(len(rows_user))
        #print(len(rows_user.columns))
        rows_user.to_csv(dataset2, mode='a', header=False)
    elif kind == 'tweets':
        df_csv = pandas.read_csv(dataset1, index_col=int(user_id_index))
        rows_user = df_csv.loc[int(user_id)]
        if isinstance(rows_user, pd.Series):
            rows_user.to_csv(dataset2, mode='a', header=False)
        else:
